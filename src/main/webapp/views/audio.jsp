...
<div class="container-fluid">
    <div class="row content">
        ...
        <div id="mainContain" class="col-sm-9">
            <div class="well">
                <button type="button" class="btn btn btn-block" onclick="document.getElementById('id01').style.display='block'">Add more audio</button>
            </div>
            <div id="id01" class="modal">
                <form id="addAudio" action="/audio/add" method="POST" class="modal-content animate" enctype="multipart/form-data">
                    <div class="modal-header">
                        <span onclick="document.getElementById('id01').style.display='none'" class="close" title="Close Modal">&times;</span>
                    </div>
                    <div class="modal-body">
                        <input id="audioAdd" class="addImage" type="file" name="audio" required/>
                        <div id="errorMessage" class="bg-danger"></div>
                    </div>
                    <div class="modal-footer">
                        <button id="sendAudio" type="submit" class="btn btn-primary">Upload</button>
                    </div>
                </form>

            </div>
            <div id="rowPhotoContent" class="row">
                <c:if test="${audio.size() == 0}">
                    <div id="haveNoFriend" class="col-sm-9"><div class="well">You don't have any audio.</div></div>
                </c:if>
                <c:if test="${audio.size() != 0}">
                    <c:forEach items="${audio}" var="audio" >
                        <div class="col-sm-9 containerAudio">
                            <div class="well photoContent">
                                <p class="lead" style="margin-bottom: 0px">${audio.name}</p><audio controls style="width: 100%" preload="none"><source src="${audio.path}" type="audio/mpeg">Audio</audio>
                                <a style="text-align: right" href="/audio/delete/${audio.id}">Delete</a>
                            </div>
                        </div>
                    </c:forEach>
                </c:if>
