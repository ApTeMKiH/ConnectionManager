...
<head>
    ...
    <link rel="stylesheet" href="/resources/css/dialogsStyle.css"/>
...
<div class="container-fluid">
    <div class="row content">
        ...
        <div id="mainContain" class="col-sm-9">
            <div class="well">
                <h2><strong>Dialogs</strong></h2>
                </div>
                <c:if test="${dialogs.size() == 0}">
                    <div id="haveNoFriend" class="col-sm-9"><div class="well">You don't have any dialogs yet.</div></div>
                </c:if>
                <c:if test="${dialogs.size() != 0}">
                    <c:forEach items="${dialogs}" var="dialog">
                        <c:if test="${dialog.userFrom.id != currentUser.id}">
                            <div onclick="location.href='/dialog/friend/${dialog.userFrom.id}'" class="container">
                                <img src="${dialog.userFrom.avatarPath}" alt="Avatar" style="width:90px">
                                <p><a href="/friend/profile/${dialog.userFrom.id}"><span>${dialog.userFrom.name} ${dialog.userFrom.surname}</span></a><a style="float: right" href="/dialog/delete/${dialog.id}"><span class="glyphicon glyphicon-remove"></span></a></p>
                                <p><span>Last message: </span>${dialog.lastMessage}</p>
                            </div>
                        </c:if>
                        <c:if test="${dialog.userTo.id != currentUser.id}">
                            <div onclick="location.href='/dialog/friend/${dialog.userTo.id}'" class="container">
                                <img src="${dialog.userTo.avatarPath}" alt="Avatar" style="width:90px">
                                <p><a href="/friend/profile/${dialog.userTo.id}"><span>${dialog.userTo.name} ${dialog.userTo.surname}</span></a><a style="float: right" href="/dialog/delete/${dialog.id}"><span class="glyphicon glyphicon-remove"></span></a></p>
                                <p><span>Last message: </span>${dialog.lastMessage}</p>
                            </div>
                        </c:if>
                    </c:forEach>
